...
class SegModel:
    ...
    def create_seg_model(self, net, n=21, backbone='mobilenetv2', load_weights=False, multi_gpu=True, infer=False, weights='cityscapes'):
        ...
        model = Deeplabv3(weights=weights, input_tensor=None, infer=infer,
...
class SegmentationGenerator(Sequence):
    ...
    def __init__(self, dataset, mode='train', subscene=None, n_classes=19, batch_size=1, resize_shape=(512,512), 
        ...
        if subscene != None:
            self.dataset = tmp[tmp['scene'] == subscene]
        else:
            self.dataset = tmp[tmp['subset'] == mode]
    ...
    def __getitem__(self, i):
        ...
        self.Y = np.zeros((self.batch_size, self.resize_shape[1] * self.resize_shape[0], 1), dtype='uint8')
        ...
        for n, (image_path, label_path) in enumerate(zip(self.image_path_list[i*self.batch_size:(i+1)*self.batch_size], 
            ...
            label = label.flatten()
            self.X[n] = image
            self.Y[n] = np.expand_dims(label, -1)
