...
def images_to_datasets(input, output):
    total_per_scene = pd.DataFrame(columns = ['type', 'image', 'scene'])
    ...
    for s in scenes_list:
        ...
        for i in images_list:
            ...
            for j in uniques:
                new_row = pd.Series({"type": j, "image": os.path.basename(i)  ,"scene": scene_name, "fullpath": i})
