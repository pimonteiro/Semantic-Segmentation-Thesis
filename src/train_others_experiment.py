...
from tensorflow.python.keras.layers import Reshape, Lambda, Input
...
def build_model(model_name, pretrained, input_size=(512,512,3)):
    ...
    try:
        ...
        model_base = sm.Unet('efficientnetb3', classes = 19, input_shape=input_size, encoder_weights=weights, activation='softmax')(img_input)
        model_base = Reshape((input_size[0]*input_size[1], 19)) (model_base)

        model = Model(img_input, model_base)   
        
        print(model.summary())
    except:
        raise Exception("No model with given backbone: ", model_name)

    model.compile(optimizer = Adam(lr=0.001, epsilon=1e-8, decay=1e-6), sample_weight_mode = "temporal",
